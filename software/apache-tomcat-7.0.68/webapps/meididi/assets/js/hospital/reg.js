var lxwm = dialog({
	...
	+				'<p>服务邮箱&nbsp;meididi_support@163.copm</p>'
	+			'</div>'
	+		'</div>'
	+		'<p class="popup_btn">'
             ...
             if (reg.test(resPhone)) {
                 $.ajax({
                   ...
                   success:function(data){
					...
					+'<p class="c_e f16">审核已通过<a href="'+json.gotoemail+'" title="" class="mail_alink" target="_blank">去邮箱</a></p>';
                            var str_3='<span class="ico_audit_notpass">×</span><p class="c_pink">'+json.name+'</p><p class="c_e f16">审核未通过<a href="javascript:;" title="" class="mail_alink" onclick="javascript: window.open(\''+json.gotoemail+'\')">去邮箱</a></p>';
                            if(json.state=="-1"){
