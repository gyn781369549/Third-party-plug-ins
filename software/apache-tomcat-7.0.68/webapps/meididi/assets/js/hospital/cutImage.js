 ...
 * 或者改动很大可以自己写ajax交互注意加关闭layer.close(Jcrop_cut_layer);图片回显加随机数
 * 例子
 ...
 */
function Jcrop_init(img_src,P_top,P_left){
	//打开裁剪框
	_showCutLayer(P_top,P_left);
	$("#cutPhoto").attr("src","../upload"+img_src);
	$("#photo").val(img_src);
	Jcrop_api = $("#cutPhoto").Jcrop({
...
function Jcrop_init_2(img_src,P_top,P_left){
	//打开裁剪框
	_showCutLayer(P_top,P_left);
	$("#cutPhoto").attr("src","../upload"+img_src);
	$("#photo").val(img_src);
 ...
 */
function Jcrop_cut_submit(previewImgId,valImgId,isReload){
	var reload=false;
	...
	$.ajax({
		...
		success:function(data){
			if("success" ==data){
				//图片会缓存加随机数
				$("#"+previewImgId).attr("src","../upload"+$("#photo").val()+"?"+new Date().getTime());
...
function Jcrop_cut_submit_no(previewImgId,isReload){
	var reload=false;
	reload=(isReload==true?true:false);
	reload=(isReload=='true'?true:false);
	$.ajax({
...
function _showCutLayer(P_top,P_left){
	...
	+'   		<input type="button" class="input_green mr20" style="border-radius: 2px;width: 204px;height: 38px;color: #fff;border: 0px;" value="确认" onclick="cut_submit()">'
	+'    		<input type="reset" class="input_green mr20" style="border-radius: 2px;width: 204px;height: 38px;color: #fff;border: 0px;" value="取消" onclick="layer.closeAll();">'
	+'		</p>'
