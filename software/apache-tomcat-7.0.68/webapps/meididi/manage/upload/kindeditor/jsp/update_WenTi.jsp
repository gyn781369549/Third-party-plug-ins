	...
	<div  >
		...
		<form action="${contextPath}/api/Hospital/themeUpload" method="post" name="picForm"
			...
			<table  cellpadding="0" cellspacing="0" >
				<tr>
					<td>标题</td>
					<td> <input type="text" style="width: 645px;" id="title" value="${result.title }" name="title" /></td>
				</tr>
				...
				<tr>
					...
					<td colspan="2"><textarea name="content" id="content"
							cols="100" rows="8"
							style="width: 500px; height: 500px; visibility: hidden;">${result.content }</textarea>
				...
				<tr>
					<td>作者</td>
					<td> <input type="text" style="width: 645px; " id="author" value=${result.author } name="author" /></td>
				</tr>
   ...
   个数<c:out value="${s.count}"/> 
    -->
		...
		<c:forEach items="${list}" var="z" varStatus="s">
    		    ...
    		    <c:forEach items="${lTipsId}" var="lz" varStatus="ls">
    		    
    			<!--   等于就停止循环-->
    		   <c:if test="${fn:containsIgnoreCase(lz, z.id)}">
    		   
    		  			 <c:set var="flag" value="false" />
    		  			 
      					     <label><input name="tipsId" id="tipsId"  type="checkbox" checked=true value='<c:out value="${z.id}"/>' />
   							 <c:out value="${z.content}"/></label> 
	   		...
	   		<c:if test="${flag }">
				...
				<tr>
					<td></td>
						...
						onclick="return isCheckForm();"  style="width: 50%; height:30px;background-color: #7FFFD4;" value="确定更新" /></td>
				</tr>
